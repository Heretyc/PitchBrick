...
7. Ensure API-triggered routines can post PR reports through an approved GitHub
   App, connector, or maintainer process. If no reporting permission exists,
   preserve reports in the Claude session and require human review of the
   session URL before merge.
8. Install the Claude GitHub App for GitHub-event routines when direct webhook
   triggers are used.
9. Keep routine repository branch pushes disabled for CI/CD routines. If a
...
- trigger on metadata-only `pull_request_target`, `workflow_dispatch`,
  `merge_group`, and default-branch push events unless owner policy adds more
  protected branches
- use least-privilege `GITHUB_TOKEN` permissions
- avoid checkout and avoid executing untrusted PR code; `pull_request_target`
  must be only a base-branch metadata bridge
...
- restrict manual `workflow_dispatch` use to the default branch unless an owner
  approves protected-branch coverage and verifies the workflow file is trusted
...
- Fork and Dependabot PRs use the metadata-only `pull_request_target` bridge.
  The routine may inspect event metadata, but it must fetch and validate the PR
  head SHA as untrusted code without exposing routine secrets to that code.
