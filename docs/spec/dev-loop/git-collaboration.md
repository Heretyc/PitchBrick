...
3. Code, CI/CD, schema, prompt/policy, and multi-file documentation changes
   intended for merge must use a short-lived topic branch and PR. Other
   non-trivial changes intended for merge also use a topic branch unless the
   owner approves an emergency protected/default-branch exception.
4. Direct protected/default-branch edits are emergency-only and require explicit
   ...
   Policy-pack installer bootstrap is a narrow exception: after explicit owner
   approval, it may create baseline and install commits on the current branch,
   must preserve user-owned dirty work, must inspect staged diffs, and must not
   push.
...
12. Use at most one writable worktree per branch. Recovery duplication must be
    detached/read-only, or temporary with explicit owner approval and immediate
    reconciliation; it must preserve the one-writable-worktree invariant.
...
24. `.github/workflows/claude-routine.yml` must run on metadata-only
    `pull_request_target`, default-branch `push`, default-branch
    `workflow_dispatch`, and `merge_group`. Additional protected-branch push
    coverage requires explicit owner configuration.
...
26. Do not use `pull_request_target` to checkout, build, test, lint, or execute
    untrusted PR code; it is allowed only as a base-branch metadata bridge.
...
1. Before work: inspect status, read `AGENTS.md`, identify owned files, and
   record all dirty/unowned paths before edits or git writes. Then identify
   which paths affect the task.
...
4. Commit: inspect staged diff, run validation, run the AGENTS-required
   contradiction-checker before commits changing executable/source code, then
   commit the smallest coherent unit only when requested or workflow-required.
