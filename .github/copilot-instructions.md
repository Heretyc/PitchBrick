...
ENTRY_LIMITS = {'AGENTS.md': 100, 'CLAUDE.md': 100, 'GEMINI.md': 100}
for p in Path('.').rglob('*'):
    ...
    if p.is_file() and (p.suffix in {'.md', '.rag'} or p.name in {'AGENTS.md','CLAUDE.md','GEMINI.md'}):
        limit = ENTRY_LIMITS.get(p.name, 200)
        assert len(p.read_text(encoding='utf-8', errors='replace').splitlines()) <= limit, p
