...
5. Use a topic branch and PR for code, CI/CD, schema, prompt/policy, or
   multi-file documentation work. Direct protected/default-branch work is
   emergency-only and needs explicit owner approval.
...
- Claude implementation routines may use `claude/` branches only when the
  normative git policy allows that tool exception.
...
- Use `pull_request_target` only for metadata-only dispatch without checkout or
